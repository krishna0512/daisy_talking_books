...
class EditorPage(TemplateView):
    ...
    def get_context_data(self):
        ...
        return context
class whole_book_xml(TemplateView):
    template_name = "whole_book_xml.html"
    def get_context_data(self):
        context = {}
        bookid = self.request.GET.get('bookid')
        title = Book.objects.get(id=bookid).title
        context['title'] = title
        context['bookid'] = bookid
        context['xml']= Book.objects.filter(id=bookid).values("daisy_xml")[0]["daisy_xml"]
        return context
...
def load_image_and_text(request):
    ...
    processed_flag = Upload.objects.filter(id=page_id).values('processed')[0]['processed']
...
#######################################
    ...
    # if page_position == 'first':
    #     xmltext = xml_preloaded_start_tags + xmlpage_number_tag + "\n" + ocrtext
    # elif page_position == 'last':
    #     xmltext = xmlpage_number_tag + "\n" + ocrtext +"\n" + xml_preloaded_last_tags
    # elif page_position == 'intermediate': 
    #     xmltext = xmlpage_number_tag + "\n" + ocrtext
#######################################
    ...
    if processed_flag == True:
        ...
        context["text"] = xmlpage_number_tag + xml_output
    else:
        ...
        context["text"] = xmlpage_number_tag +ocr_output
